...
where $\lambda$ is the bulk thermal conductivity of the rock and $\mathrm{d}T/\mathrm{d}z$ the vertical temperature gradient. The gradient is measured directly in high-precision borehole temperature logs sampled every 1 cm and averaged over a 1 m moving window. On the other hand $\lambda$ is not measurable in situ and must instead be determined on core samples in the laboratory. @fuchs2010 measured it by optical scanning [@popov1999], which records the maximum temperature rise $\Theta$ on the heating line behind a moving laser heat source (@fig-optical-scanning) and converts it to a conductivity by also scanning a reference standard of known conductivity $\lambda_s$ alongside the sample:
$$
\lambda = \lambda_s \frac{\Theta_s}{\Theta}
$$ {#eq-optical}

[@fuchs2010, Eq. 2]. Each sample was scanned at least three times, dry and water-saturated, and the results corrected to in-situ temperature after @sass1992. The corrected, water-saturated values are used as the input for the re-analysis in this project. 


![The optical-scanning apparatus, after @popov1999. A reference standard of known conductivity (1) heads a row of samples (2) laid along the scanning line; the heat source (3) and the infrared radiometers (4) sit on a carriage that travels beneath the whole row in a single pass. Because source power and source–sensor distance are then identical over standard and samples, they cancel in the ratio $\Theta_\text{R}/\Theta$ and no absolute calibration is needed.](figures/popov_os_apparatus.png){#fig-optical-scanning width=80%}

@fuchs2010 derive matrix conductivity, $\lambda_{\text{matrix}}$, in two independent ways. The first inverts the geometric-mean mixing law [@fuchs2010, Eq. 3]:
$$
...
using the measured bulk conductivity $\lambda_{\text{satM}}$ and the effective porosity. Their second approach computes $\lambda_{\text{matrix}}$ as a volume-weighted geometric mean of mineral conductivities obtained from prior literature:

$$
\lambda_{\text{satM}} = \Pi_{i=1}^{n} \cdot \lambda_{\text{i}}^{\phi}
$$ {#eq-geom2}

[@fuchs2010, Eq. 5]. This report reproduces their first approach.
...
Of the two terms in @eq-fourier only the gradient is easy: temperature logs constrain it
tightly, while $\lambda$ must be carried from a few decimetres of core to hundreds of
metres of rock, so it is $\lambda$ that sets the uncertainty in $q$ [@beardsmore2001;
@clauser1995]. Core coverage compounds this systematically, since coring targets reservoir
intervals and leaves the overburden supplying most of the thermal resistance sampled least.
Porosity, density, sonic and gamma logs, by contrast, run continuously and exist in wells
that were never cored. If $\lambda$ could be predicted from them, sparse core measurements
would become a continuous conductivity profile and, with the temperature log, a continuous
heat flow — provided the relationship is strong enough at sample scale to be useful.
...
avoided by laying a reference standard of known $\lambda_\text{R}$ in the same row as the
samples and scanning both in one pass, so that $\lambda = \lambda_\text{R}\,\Theta_\text{R}/\Theta$. The
